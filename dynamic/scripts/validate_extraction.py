        ...
        for model in configured_models(c):
            ...
            for regime in regimes:
                ...
                with np.load(path) as z:
                    ...
                    for key,shape in expected_shapes(model,c,n).items():
                        expected.add(key);assert z[key].shape==shape,(key,z[key].shape,shape)
                    from src.feature_io import json_safe
                    assert r['transform']==json_safe(transform_for(model).metadata())
                    assert set(z.files)==expected
                    for key in expected-{'metadata_json'}:nonfinite+=int((~np.isfinite(z[key])).sum())
                if MODELS[model]['family']=='geometry':
                    errs=r['debug']['register_only_patch_max_errors'];assert set(errs)==set(map(str,c[model].get('register_only_layers',[])))
                    maxerror=max(maxerror,max(errs.values(),default=0.));spec=MODELS[model];assert r['debug']['dense_shape']==[1,spec['dense_dim'],spec['dense_hw'],spec['dense_hw']]
                assert r['debug']['clean_patches']>0
                checked.append(r['path'])
    assert nonfinite==0 and maxerror<=1e-6
    result={'passed':True,'stage':a.stage,'config_hash':config_hash(c),'shards_checked':len(checked),'expected_shards':len(seqs)*shards_per_sequence(c),'nonfinite':nonfinite,'register_only_patch_max_error':maxerror,'checked':checked}
