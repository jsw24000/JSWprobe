...
class AggregatorBase(nn.Module, ABC):
    ...
    def __init__(
        ...
        if self.pretrained_path:
            logger.info(f"pretrained_path: {self.pretrained_path}")
    ...
    def _build_patch_embed(
        ...
        if "conv" in patch_embed:
            ...
            # Load DINOv2 weights only when an explicit preinit path is provided.
            if pretrained_path:
                try:
                ...
                logger.info("No DINOv2 preinit path provided; expecting model checkpoint load.")
                self._dino_checkpoint = None
